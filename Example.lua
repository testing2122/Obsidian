...
local Tabs = {
	...
	Rendering = Window:AddTab("Rendering", "image"),
...
-- IMAGE AND 3D RENDERING SUPPORT
local ImageGroupBox = Tabs.Rendering:AddLeftGroupbox("Image Rendering", "image")
local ModelGroupBox = Tabs.Rendering:AddRightGroupbox("3D Model Rendering", "box")
...
local function createimg(imgurl, pos, size, transparency)
	local img = Drawing.new("Image")
	img.Data = game:HttpGet(imgurl)
	img.Position = pos or Vector2.new(100, 100)
	img.Size = size or Vector2.new(256, 256)
	img.Transparency = transparency or 1
	img.Visible = true
	return img
end

local function removeimg(img)
	if img then
		img:Remove()
	end
end
...
local function create3dmdl(mdlid, pos, rot, scale)
	local mdl = Instance.new("Part")
	mdl.Name = "RenderedModel"
	mdl.Anchored = true
	mdl.CanCollide = false
	mdl.Position = pos or Vector3.new(0, 10, 0)
	mdl.Rotation = rot or Vector3.new(0, 0, 0)
	mdl.Size = scale or Vector3.new(4, 4, 4)
	mdl.Material = Enum.Material.Neon
	mdl.BrickColor = BrickColor.new("Bright blue")
	
	local mesh = Instance.new("SpecialMesh")
	mesh.MeshType = Enum.MeshType.FileMesh
	mesh.MeshId = "rbxassetid://" .. tostring(mdlid)
	mesh.Parent = mdl
...
ImageGroupBox:AddToggle("ImgRenderEnabled", {
	Text = "Enable Image Rendering",
	Default = false,
	Callback = function(val)
		if not val then
			for i, img in pairs(imgrendr) do
				removeimg(img)
			end
			imgrendr = {}
		end
...
ImageGroupBox:AddSlider("ImgPosX", {
...
ImageGroupBox:AddSlider("ImgPosY", {
...
ImageGroupBox:AddSlider("ImgSizeX", {
	Text = "Image Width",
	Default = 256,
	Min = 16,
	Max = 512,
...
ImageGroupBox:AddSlider("ImgSizeY", {
	Text = "Image Height",
	Default = 256,
	Min = 16,
	Max = 512,
...
ImageGroupBox:AddSlider("ImgTransparency", {
...
ImageGroupBox:AddButton({
	Text = "Render Image",
	Func = function()
		if Toggles.ImgRenderEnabled.Value then
			local pos = Vector2.new(Options.ImgPosX.Value, Options.ImgPosY.Value)
			local size = Vector2.new(Options.ImgSizeX.Value, Options.ImgSizeY.Value)
			local img = createimg(Options.ImgURL.Value, pos, size, Options.ImgTransparency.Value)
			table.insert(imgrendr, img)
		end
	end
})
...
-- 3D model rendering controls
ModelGroupBox:AddToggle("MdlRenderEnabled", {
	Text = "Enable 3D Model Rendering",
	Default = false,
	Callback = function(val)
		if not val then
			for i, mdl in pairs(mdlrendr) do
				removemdl(mdl)
			end
			mdlrendr = {}
		end
...
ModelGroupBox:AddInput("MdlID", {
	Default = "1234567890",
	Numeric = true,
	Text = "Model Asset ID",
	Placeholder = "Enter model ID...",
	Callback = function(val)
		-- ID validation can be added here
	end
})

ModelGroupBox:AddSlider("MdlPosX", {
	Text = "Model X Position",
	Default = 0,
	Min = -100,
	Max = 100,
	Rounding = 1
...
ModelGroupBox:AddSlider("MdlPosY", {
	Text = "Model Y Position",
	Default = 10,
	Min = 0,
	Max = 100,
	Rounding = 1
...
ModelGroupBox:AddSlider("MdlPosZ", {
	Text = "Model Z Position",
	Default = 0,
	Min = -100,
	Max = 100,
	Rounding = 1
...
ModelGroupBox:AddSlider("MdlRotX", {
	Text = "Model X Rotation",
	Default = 0,
	Min = 0,
	Max = 360,
	Rounding = 0
...
ModelGroupBox:AddSlider("MdlRotY", {
	Text = "Model Y Rotation",
	Default = 0,
	Min = 0,
	Max = 360,
	Rounding = 0
...
ModelGroupBox:AddSlider("MdlRotZ", {
	Text = "Model Z Rotation",
	Default = 0,
	Min = 0,
	Max = 360,
	Rounding = 0
...
ModelGroupBox:AddSlider("MdlScale", {
	Text = "Model Scale",
	Default = 4,
	Min = 0.1,
	Max = 20,
...
ModelGroupBox:AddButton({
	Text = "Render 3D Model",
	Func = function()
		if Toggles.MdlRenderEnabled.Value then
			local pos = Vector3.new(Options.MdlPosX.Value, Options.MdlPosY.Value, Options.MdlPosZ.Value)
			local rot = Vector3.new(Options.MdlRotX.Value, Options.MdlRotY.Value, Options.MdlRotZ.Value)
			local scale = Vector3.new(Options.MdlScale.Value, Options.MdlScale.Value, Options.MdlScale.Value)
			local mdl = create3dmdl(Options.MdlID.Value, pos, rot, scale)
			table.insert(mdlrendr, mdl)
		end
	end
})
